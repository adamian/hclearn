...
#matplotlib.use('Agg')
import numpy as np
from locationLuke import Location # Devel modifiction Luke
...
# LB: SEND string as path into saveFolder to save data and figs.....


def plotResults(paths, hist, dictGrids, b_useNewDG, learningRate, note="", saveFolder=None):
    ...
    #N_places = (1+2*paths.N_mazeSize)**2 # DO NOT USE
...
#        loc=Location()
        ...
        loc.setGrids(hist.ca1s[t].grids) # Luke DEVEL modified
...
#    plt.subplot(4,2, (b_col2)+3)
    ...
    plt.hold(True)
    plt.plot(paths.posLog[:,1], 'k')
    plt.ylabel('y location')

    #head directions
#    plt.subplot(4,2, (b_col2)+5)
    plt.subplot(4,1, 3)
    plt.plot(paths.posLog[:,2], 'k')   #ground truth
...
#    plt.subplot(4,2, (b_col2)+7)
    ...
    b_losts = (np.sum(xys_bel!=paths.posLog[:,0:2], 1)!=0)
    
    # COMMENTED OUT SAVING AS THE PATH IS BUGGY IN NOTEBOOK
    if saveFolder is not None:
        dir_name = os.path.join(saveFolder,"Results/")        
        if not os.path.exists(dir_name):
            os.makedirs(dir_name)  
        strFname="run_"+hist.str_title+"LR_%d_DG_%s_%s.eps" % (learningRate*1000, b_useNewDG, note)
        plt.savefig(os.path.join(dir_name,strFname))
...
def plotErrorsN(hists, losts, learningRate, subTest = True, surfTest = True, placeTest=True, note="", saveFolder=None):
...
#        lost_titles.append(losts[lost_key].str_title)
    ...
    if subTest:   #sense errors
        ...
        plt.title(ti)
        figName = "err_senses_LR%d_DG%s_%s" % (learningRate*1000, surfTest, note)
       # COMMENTED AS RELATIVE PATHS HAVEN'T BEEN SORTED
#        strFname="run_"+hist.str_title+"LR_%d_DG_%s_%s.eps" % (learningRate*1000, b_useNewDG, note)
#        plt.savefig(figName+".eps")
        if saveFolder is not None:
            plt.savefig(os.path.join(dir_name,figName))          
            np.save(os.path.join(dir_name,figName), np.vstack((np.array(sub_mu), np.array(sub_sigma))))
    ...
    if surfTest:
        ...
        plt.title(ti)
        figName= "err_surf_LR%d_DG%s_%s" % (learningRate*1000, surfTest, note)
        
        if saveFolder is not None:
            plt.savefig(os.path.join(dir_name,figName))          
            np.save(os.path.join(dir_name,figName), hist_accuracy)        
#        plt.savefig(figName + ".eps")
#        np.save(figName, hist_accuracy)
    ...
    if placeTest:
        ...
        plt.title(ti)
        figName = "err_places_LR%d_DG%s_%s" % (learningRate*1000, surfTest, note)
        
        if saveFolder is not None:
            plt.savefig(os.path.join(dir_name,figName))          
            np.save(os.path.join(dir_name,figName), np.vstack((lost_mu, lost_sigma)))     
