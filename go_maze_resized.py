...
if b_plot:
 ...
 #   np.save('tWO',WO)
    ...
    ## Learned values
    losts[('Learned')],xys1 = plotResults(path_config, hists['Learned'], dictGrids, b_useNewDG, learningRate, note=anote, saveFolder=fullImageFolder)
    ## randomised weights
    if ctrl_randomised_random:    
        losts[('Random')],xys2 = plotResults(path_config, hists['Random'], dictGrids, b_useNewDG, learningRate, note=anote, saveFolder=fullImageFolder)
    if ctrl_handset_ideal:
        losts[('Ideal')],xys3 = plotResults(path_config, hists['Ideal'], dictGrids, b_useNewDG, learningRate, note=anote, saveFolder=fullImageFolder)
    if ctrl_randomised_zero:
        losts[('RandomZero')],xys4 = plotResults(path_config, hists['RandomZero'], dictGrids, b_useNewDG, learningRate, note=anote, saveFolder=fullImageFolder)
    #savefig('out/run.eps')
    ...
    plotErrorsN(hists, losts, learningRate, surfTest=b_useNewDG, note=anote, saveFolder=fullImageFolder)
    ##figure()
    ##drawMaze()
    ##hold(True)
...
if b_inference_test:
    ...
    if ctrl_randomised_zero: 
        random.seed(SEED) ;  np.random.seed(SEED)
        hists_path['RandomZero'] = makeMAPPredictions(testPath,dictGrids, dictSenses, WB_rand0,  WR_rand0,  WS_rand0, WO_rand0, dghelper, b_obsOnly=b_obsOnly, b_usePrevGroundTruthCA3=b_usePrevGroundTruthCA3,  b_useGroundTruthGrids=b_useGroundTruthGrids,  b_useSub=b_useSub, str_title="Random", b_learn=b_learn)
print "DONE"

if b_plot:
    losts_path=OrderedDict()
    anote = "obsonly%s_gndCA3%s_gndgrid%s_sub%s_bl%s_MERGED0102_SUBT026" % (b_obsOnly, b_usePrevGroundTruthCA3, b_useGroundTruthGrids, b_useSub, b_learn)
    losts_path['Learned'],xys1 = plotResults(testPath, hists_path['Learned'], dictGrids, b_useNewDG, learningRate, note=anote, saveFolder=fullImageFolder)
    if ctrl_randomised_random: 
        losts_path['Random'],xys2 = plotResults(testPath, hists_path['Random'], dictGrids, b_useNewDG, learningRate, note=anote, saveFolder=fullImageFolder)
    if ctrl_handset_ideal:
        losts_path['Ideal'],xys3 = plotResults(testPath,hists_path['Ideal'], dictGrids, b_useNewDG, learningRate, note=anote, saveFolder=fullImageFolder)    
    if ctrl_randomised_zero:
        losts_path['RandomZero'],xys3 = plotResults(testPath,hists_path['RandomZero'], dictGrids, b_useNewDG, learningRate, note=anote, saveFolder=fullImageFolder)        
    plotErrorsN(hists_path, losts_path, learningRate, surfTest=b_useNewDG, note=anote, saveFolder=fullImageFolder)
    show()
