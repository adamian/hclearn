...
dir_name_overall="division_street_1"
                ...
                if os.path.getsize(outfile) == 8381:  # bytes
                    ...
                    if imagery_hits >1:  # changed here as imagery starts at second location to prevent duplicate images! 
                        if filecmp.cmp(tempfile,outfile):#os.path.getsize(outfile)==tempfilesize: #
                            ...
                            print "Files same size! -> removing, adding 10% extra step to lat / lon data...."
                            # Add on 10% extra of step lon and lat to                             
                            steps_lat_all[imagery_hits-steps_lat_all.size:]=steps_lat_all[imagery_hits-steps_lat_all.size:]+numpy.tile(step_lat*0.15,[1,steps_lat_all[imagery_hits-steps_lat_all.size:].size])
                            steps_lon_all[imagery_hits-steps_lon_all.size:]=steps_lon_all[imagery_hits-steps_lon_all.size:]+numpy.tile(step_lon*0.15,[1,steps_lon_all[imagery_hits-steps_lon_all.size:].size])
...
# Find locations in 
primary_street_intersection=numpy.empty([9,2])
primary_street_intersection_x=numpy.empty([9,1])
...
vertical_y_street_intersection=numpy.empty([9,2])
vertical_y_street_intersection_x=numpy.empty([9,1])
...
for find_close in range(0,primary_street_intersection[:,1].size):
    ...
    primary_street_intersection_x[find_close]=numpy.abs(steps_lon_division-primary_street_intersection[find_close,1]).argmin()-1

grid_start=numpy.zeros([1,2])

# Has grid start and grid end of each intersection!!!
x_grid_index_intersections=numpy.zeros([vertical_y_street_intersection[:,1].size,4])
...
grid_intersection_ref=numpy.empty([4,len(secondary_street_intersections)],dtype='f4')        
...
#numpy.sort(grid_intersection_ref,axis=1)
I = numpy.argsort(grid_intersection_ref[:,0])
grid_intersection_ref=grid_intersection_ref[I,:]

# Has grid start and grid end of each intersection!!!
y_grid_index_intersections=numpy.zeros([grid_intersection_ref.shape[0],4]) # add another row for street end (none as west street finishes)
